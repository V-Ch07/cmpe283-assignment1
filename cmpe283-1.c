...
*/
struct capability_info proc_primary[22] =
{
...
*/
struct capability_info proc_secondry[27] =
{
...
exit controls
*/
struct capability_info exits_vm[14] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Procbased primary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(proc_primary, 22, lo, hi);
	
	/* Procbased secondary controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Procbased secondary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(proc_secondry, 27, lo, hi);
	
	/* exit controls */
	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exits_vm, 14, lo, hi);
